...
while ( have_posts() ) :
    ...
    <?php if ( null !== $price_standard ) : ?>
        <?php if ( null !== $price_reserved && $price_reserved < $price_standard ) : ?>
            ...
            <div class="bsn-price-note">Prezzo dedicato al tuo profilo cliente. Per noleggi piu lunghi il costo medio si riduce con il calcolo scalare.</div>
        <?php else : ?>
            <div class="bsn-price-main">Tariffa 1 giorno: <?php echo esc_html( number_format_i18n( $price_standard, 2 ) ); ?> EUR</div>
            <div class="bsn-price-note">Per noleggi piu lunghi il costo medio si riduce con il calcolo scalare.</div>
        <?php endif; ?>
    <?php else : ?>
    ...
    <?php endif; ?>
                        ...
                        <div class="bsn-prodotto-logistics">
                            ...
                            <div><strong>Quantita minima:</strong> <?php echo esc_html( $min_qty ); ?></div>
                        ...
                        <div class="bsn-prodotto-date-box">
                            <div class="bsn-date-grid">
                                <div class="bsn-public-date-field" data-date-lock-trigger="1">
                                    ...
                                    <input type="date" id="bsn-public-data-ritiro" value="<?php echo esc_attr( $data_ritiro ); ?>">
                                </div>
                                <div class="bsn-public-date-field" data-date-lock-trigger="1">
                                    <label for="bsn-public-data-riconsegna">Data riconsegna</label>
                                    <input type="date" id="bsn-public-data-riconsegna" value="<?php echo esc_attr( $data_riconsegna ); ?>">
                                </div>
                            </div>
                            ...
                            <div class="bsn-date-actions">
                                <button type="button" class="bsn-public-btn" id="bsn-public-check-availability">Verifica disponibilita</button>
                                <button type="button" class="bsn-public-btn bsn-public-btn-secondary" id="bsn-public-add-to-cart" disabled hidden>Aggiungi al preventivo</button>
                                <a class="bsn-public-btn bsn-public-btn-ghost" href="<?php echo esc_url( $quote_cart_url ); ?>">Vai al carrello</a>
                            </div>
                            <div id="bsn-public-cart-note" class="bsn-public-inline-note" hidden></div>
                        </div>

                        <div id="bsn-public-quote-preview" class="bsn-public-quote-box">
                            <div class="bsn-price-label">Anteprima prezzo</div>
                            <div class="bsn-price-note">Inserisci date e quantita per ottenere una stima pre-carrello con la logica prezzo pubblica standard.</div>
                        </div>
                        <div class="bsn-public-disclaimer">
                            Il carrello preventivo usa gia date globali uniche. Dal carrello puoi gia inviare la richiesta; i servizi extra arriveranno nello step successivo.
                        </div>
    ...
    (function() {
        ...
        var lockedDatesMessage = 'Le date di questo noleggio sono gia state definite dal carrello. Per modificarle, aggiorna le date direttamente nel carrello.';
        var state = {
            cartLocked: false,
            verified: false,
        ...
        if (!btn || !addBtn || !inputRitiro || !inputRiconsegna || !inputQty || !result || !quoteBox || !cartNote) {
            return;
        }
        ...
        function validateDateRange() {
            if (inputRitiro.value && inputRiconsegna.value && inputRitiro.value > inputRiconsegna.value) {
                ...
                quoteBox.innerHTML = '<div class="bsn-price-label">Anteprima prezzo</div><div class="bsn-price-note">Correggi le date per ottenere la stima.</div>';
        ...
        function renderQuotePreview(data) {
            if (!data || data.success === false) {
                hideAddButton();
                quoteBox.innerHTML = '<div class="bsn-price-label">Anteprima prezzo</div><div class="bsn-price-note">' + escHtml((data && data.message) ? data.message : 'Impossibile calcolare la stima.') + '</div>';
                return false;
            }
        ...
        function renderAvailability(data, qty) {
            ...
            result.innerHTML =
                '<div class="bsn-public-availability-live">' +
                    ...
                    '<div class="bsn-live-note">Per quantita superiori contattaci: possiamo verificare disponibilita aggiuntiva o soluzioni alternative.</div>' +
                '</div>' +
                warningBlocks.join('');
        ...
        function runAvailabilityCheck(options) {
            ...
            return fetch(root + 'public-products/availability?' + params.toString(), {
                ...
                .catch(function() {
                    ...
                    quoteBox.innerHTML = '<div class="bsn-price-label">Anteprima prezzo</div><div class="bsn-price-note">Errore nel calcolo della stima.</div>';
                    return false;
                })
                .finally(function() {
                    if (!state.cartLocked) {
        ...
        inputQty.addEventListener('change', function() {
            getQtyValue();
            if (state.cartLocked && inputRitiro.value && inputRiconsegna.value) {
        ...
        addBtn.addEventListener('click', function() {
            ...
            fetchCartState().then(function(cart) {
                ...
                if (!state.cartLocked && !state.verified) {
                    alert('Verifica la disponibilita prima di aggiungere il prodotto al preventivo.');
                    return;
                }
                if (qty > state.lastAvailableUnits) {
                    alert('Quantita richiesta superiore a quella disponibile per il periodo selezionato.');
