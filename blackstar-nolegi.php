...
function bsn_prepare_quote_cart_service( $service, $cart_dates = array() ) {
    ...
    if ( $mode === 'delivery_install_tech' ) {
        if ( $service['event_date'] === '' ) {
            $errors[] = 'Inserisci la data dell\'evento.';
        }
    ...
    if ( $distance_km !== null ) {
        if ( $mode === 'delivery_only' ) {
            ...
            $event_start = bsn_combine_service_event_datetime( $service['event_date'], $service['event_start_time'] );
            $event_end = bsn_combine_service_event_datetime( $service['event_date'], $service['event_end_time'] );
            $event_hours = 0.0;
            if ( $event_start !== '' && $event_end !== '' ) {
                $start_ts = strtotime( $event_start );
                $end_ts = strtotime( $event_end );
                if ( $start_ts && $end_ts && $end_ts > $start_ts ) {
                    $event_hours = round( ( $end_ts - $start_ts ) / HOUR_IN_SECONDS, 2 );
                }
            }
            $tech_base = ( $event_hours > 0 && $event_hours <= $pricing['tech_half_day_hours'] )
                ? $pricing['tech_half_day']
                : $pricing['tech_full_day'];
            $estimated_total = $tech_base + ( $distance_km * $pricing['tech_transfer_km_rate'] );
        }
    }
...
function bsn_api_public_product_availability( WP_REST_Request $request ) {
    ...
    return rest_ensure_response(
        bsn_get_public_product_availability( $product_id, $data_ritiro, $data_riconsegna )
    );
}
...
function bsn_render_public_product_availability_html( $product_id, $data_ritiro = '', $data_riconsegna = '', $args = [] ) {
    ...
    if ( $show_context ) {
        if ( $data_ritiro !== '' && $data_riconsegna !== '' ) {
            $html .= '<span style="font-size:0.92em; color:#5d697a;">Periodo verificato: ' . esc_html( $data_ritiro ) . ' - ' . esc_html( $data_riconsegna ) . '</span>';
...
function bsn_build_frontend_service_request_lines( $service_snapshot ) {
    ...
    $lines = array();
    $push_line = function( $code, $price, $label ) use ( &$lines, $service_article_map, $mode ) {
    ...
    if ( $mode === 'delivery_only' ) {
        ...
        $push_line( 'SERV-KM-CONSEGNA', $distance_component * 0.60, 'Solo consegna - km' );
        if ( ! empty( $service_snapshot['service_delivery_return_requested'] ) ) {
            $push_line( 'SERV-RITIRO', 40.0, 'Ritiro finale' );
            $push_line( 'SERV-KM-AR', $distance_component * 0.60, 'Ritiro finale - km' );
        }
    } elseif ( $mode === 'delivery_install' ) {
        $push_line( 'SERV-MONTAGGIO-1TEC', 100.0, 'Trasporto con montaggio' );
        $push_line( 'SERV-KM-CONSEGNA', $distance_component * 0.60, 'Trasporto con montaggio - km' );
        if ( ! empty( $service_snapshot['service_dismantling_requested'] ) ) {
            $push_line( 'SERV-SMONTAGGIO', 100.0, 'Smontaggio e ritiro' );
            $push_line( 'SERV-KM-AR', $distance_component * 0.60, 'Smontaggio e ritiro - km' );
        }
    } elseif ( $mode === 'delivery_install_tech' ) {
        $is_half_day = false;
        if ( ! empty( $service_snapshot['service_event_start'] ) && ! empty( $service_snapshot['service_event_end'] ) ) {
            $start_ts = strtotime( (string) $service_snapshot['service_event_start'] );
            $end_ts = strtotime( (string) $service_snapshot['service_event_end'] );
            if ( $start_ts && $end_ts && $end_ts > $start_ts ) {
                $hours = ( $end_ts - $start_ts ) / HOUR_IN_SECONDS;
                $is_half_day = $hours <= bsn_get_public_service_pricing_config()['tech_half_day_hours'];
            }
        }
        ...
        $push_line( 'SERV-TRASFERTA', $distance_component * 1.20, 'Trasferta tecnica' );
    }
...
function bsn_get_noleggio_service_summary_data( $service_snapshot ) {
    ...
    if ( $event_start !== '' && $event_end !== '' ) {
        $event_date = bsn_format_date_it( $event_start );
        $event_start_time = substr( $event_start, 11, 5 );
        $event_end_time = substr( $event_end, 11, 5 );
        $event_label = trim( $event_date . ' ' . $event_start_time . ' - ' . $event_end_time );
    }
    ...
    if ( $mode === 'pickup' ) {
        ...
        if ( $mode === 'delivery_only' ) {
            $lines[] = array(
                'label' => 'Ritiro finale incluso',
                'value' => bsn_get_public_service_yes_no_label( $service_snapshot['service_delivery_return_requested'] ?? 0 ),
            );
        } elseif ( $mode === 'delivery_install' ) {
            $lines[] = array(
                'label' => 'Smontaggio e ritiro',
                'value' => bsn_get_public_service_yes_no_label( $service_snapshot['service_dismantling_requested'] ?? 0 ),
            );
        } elseif ( $mode === 'delivery_install_tech' && $event_label !== '' ) {
            $lines[] = array(
                'label' => 'Evento',
                'value' => $event_label,
            );
...
function bsn_prepare_noleggio_document_item_row( $art, $articolo_db, $categoria, $giorni, $fattore_giorni_scalare, $use_frontend_snapshot = false ) {
    ...
    $is_service_internal = ! empty( $art['service_internal'] );
...
function bsn_get_noleggio_document_totals_context( $noleggio, $subtotale, $regime ) {
    ...
    if ( $is_frontend_quote ) {
        ...
        return array(
            ...
            'pricing_note' => (string) (
                $snapshot_prezzi['pricing_note']
                ?? 'Importi del catalogo online, iva esclusa. Eventuali imposte, servizi extra e adeguamenti saranno verificati dall\'operatore.'
            ),
        );
    }

    $importo_regime = round( $subtotale * ( (float) $regime / 100 ), 2 );
    $totale_noleggio = round( $subtotale + $importo_regime, 2 );
    ...
    return array(
        ...
        'totale_label' => 'Totale noleggio',
        'totale_finale' => $totale_finale,
        'sconto_globale' => round( $totale_finale - $totale_noleggio, 2 ),
        'pricing_note' => '',
    );
}
...
function bsn_render_finalizza_noleggio_page( $args = [] ) {
    ...
    $subtotale = 0.0;
    $use_frontend_snapshot = bsn_is_frontend_quote_noleggio( $noleggio );
                ...
                '<section class="bsn-public-card bsn-quote-cart-service-box">' +
                    ...
                    '<div class="bsn-service-panel" data-service-mode="delivery_only">' +
                        '<div class="bsn-service-copy">' +
                            ...
                            '<p>Formula V1: consegna = 40 + (km x 0,60). Se aggiungi anche il ritiro finale, la tratta viene raddoppiata.</p>' +
                        '</div>' +
                    ...
                    '<div class="bsn-service-panel" data-service-mode="delivery_install">' +
                        '<div class="bsn-service-copy">' +
                            ...
                            '<p>Formula V1: 100 + (km x 0,60). Se aggiungi smontaggio e ritiro finale, la tratta viene raddoppiata.</p>' +
                        '</div>' +
                        '<div class="bsn-service-form-grid">' +
                            '<label class="bsn-service-field bsn-service-field-full">Indirizzo location' +
                ...
                '<div class="bsn-quote-cart-layout">' +
                    '<section class="bsn-public-card">' +
                        '<div class="bsn-quote-cart-dates">' +
                            ...
                            '</div>' +
                        '</div>' +
                        '<div class="bsn-quote-cart-date-meta">Le date qui impostate valgono per tutto il preventivo. Se ti servono periodi diversi, crea una seconda richiesta separata.</div>' +
        ...
        app.addEventListener('click', function(event) {
            ...
            if (submitBtn) {
                ...
                callApi('quote-cart/submit', 'POST', {
                    ...
                    service_mode: collectServicePayload().service_mode,
                    service_delivery_return_requested: collectServicePayload().service_delivery_return_requested,
                    service_dismantling_requested: collectServicePayload().service_dismantling_requested,
                    service_ack_delivery_only_terms: collectServicePayload().service_ack_delivery_only_terms,
                    service_location_address: collectServicePayload().service_location_address,
                    service_onsite_contact_name: collectServicePayload().service_onsite_contact_name,
                    service_onsite_contact_phone: collectServicePayload().service_onsite_contact_phone,
                    service_logistics_ztl: collectServicePayload().service_logistics_ztl,
                    service_logistics_stairs: collectServicePayload().service_logistics_stairs,
                    service_logistics_lift: collectServicePayload().service_logistics_lift,
                    service_logistics_tight_access: collectServicePayload().service_logistics_tight_access,
                    service_logistics_walk_distance: collectServicePayload().service_logistics_walk_distance,
                    service_notes: collectServicePayload().service_notes,
                    service_event_date: collectServicePayload().service_event_date,
                    service_event_start_time: collectServicePayload().service_event_start_time,
                    service_event_end_time: collectServicePayload().service_event_end_time
                }).then(function(response) {
                    submitNoteDraft = '';
...
function bsn_api_noleggi_dettaglio( WP_REST_Request $request ) {
    ...
    foreach ( $articoli as $a ) {
        ...
        $articoli_out[] = [
            ...
            'service_component_label' => (string) ( $a['service_component_label'] ?? '' ),
