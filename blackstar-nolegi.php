...
function bsn_api_quote_cart_submit( WP_REST_Request $request ) {
    $cart_state = bsn_quote_cart_reprice( bsn_get_quote_cart() );
...
function bsn_prepare_noleggio_document_item_row( $art, $articolo_db, $categoria, $giorni, $fattore_giorni_scalare, $use_frontend_snapshot = false ) {
    ...
    $codice = $public_code !== '' ? $public_code : (string) ( $articolo_db['codice'] ?? '' );
    ...
    $external_url = $public_permalink !== '' ? $public_permalink : (string) ( $articolo_db['external_product_url'] ?? '' );
    $external_image = $public_image_url !== '' ? $public_image_url : (string) ( $articolo_db['external_image_url'] ?? '' );
            ...
            app.innerHTML =
                ...
                '<div class="bsn-quote-cart-layout">' +
                    ...
                    '<aside class="bsn-public-card bsn-quote-cart-summary">' +
                        ...
                        '<div class="bsn-public-quote-total"><strong class="bsn-public-quote-total-label">Totale stimato:</strong> <strong class="bsn-public-quote-total-value">' + euro(data.totale_stimato || 0) + '</strong></div>' +
                        '<div class="bsn-price-note">Stima ' + escHtml(getCategoryLabel(data.categoria_cliente || 'standard')) + ', iva esclusa. I servizi extra arriveranno in uno step successivo, ma da qui puoi gia ' + (data.editing && data.editing.noleggio_id ? 'aggiornare la richiesta' : 'inviare la richiesta') + '.</div>' +
...
function bsn_api_noleggi_dettaglio( WP_REST_Request $request ) {
    ...
    foreach ( $articoli as $a ) {
        ...
        $nome       = (string) ( $a['public_product_title'] ?? '' );
        $codice     = (string) ( $a['public_product_code'] ?? '' );
        $ubicazione = '';
        $correlati  = [];
                                        ...
                                        <td>
                                            ...
                                            <details class="bsn-auth-order-details">
                                                ...
                                                <div class="bsn-auth-order-details-body">
                                                    ...
                                                    <?php endif; ?>
                                                    <div>
                                                        <strong>Materiali collegati</strong>
